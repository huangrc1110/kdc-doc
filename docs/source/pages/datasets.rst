...
当前工业场景下，机器人多模态数据集存在三大痛点：
    1. 任务单一性：多数数据集仅覆盖单一任务，缺乏多任务协同的复杂场景；
    2. 模态割裂：常见数据集未实现视觉、传感器、控制信号的多模态同步；
    3. 工业适配不足：实验室环境数据与工业真实场景存在性能与鲁棒性差距。

本次大赛发布首个面向真实工业场景、覆盖多任务的大规模机器人多模态数据集，填补了工业领域机器人具身智能研究的空白。数据集聚焦实际工业操作链路中的关键能力，包含4个核心任务（单任务样本量均超1万条），总体数据规模超过40,000条，累计时长突破100小时，为模仿学习、视觉-语言-动作模型（VLA / RT 系列方向）、策略迁移与泛化评测提供坚实基础。

本赛事通过数据集特性设计，为参赛者提供以下优势：
    - 零预处理开发：标准化hdf5/lerobot格式可直接用于训练，降低算法验证门槛；
    - 全栈研究支持：rosbag格式保留原始信号，支持从底层特征提取到上层策略优化的全链路创新；
    - 快速迭代能力：Baseline代码与文档覆盖主流算法，参赛者可聚焦核心算法改进；
    - 工业落地潜力：数据集直指工业场景痛点，研究成果可直接迁移至生产环境。

现在在赛事网站上面下载的数据为模拟器使用的rosbag包，同时 `kuavo_data_challenge <https://github.com/LejuRobotics/kuavo_data_challenge>`_ 仓库中提供了rosbag转lerobot工具。下面对rosbag和lerobot两种格式数据进行介绍：
